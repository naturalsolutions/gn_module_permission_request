...
class PermissionRequest(DB.Model):
    ...
    def _ref_permission(self):
        """First permission in the list, used as reference for shared properties."""
    ...
    def created_on(self, value):
        ...
        dt = None if value is None else (value if isinstance(value, datetime) else datetime.combine(value, datetime.min.time()))
    ...
    def created_on(cls):
        return (
            ...
            .where(
                Permission.id_permission == sa.select(_cor_permission_request_permission.c.id_permission)
                .where(_cor_permission_request_permission.c.id_permission_request == cls.id_permission_request)
                .order_by(_cor_permission_request_permission.c.id_permission)
                .limit(1)
                .scalar_subquery()
            )
            .scalar_subquery()
    ...
    def validated(self):
        ref = self._ref_permission
        return ref.validated if ref is not None else None
    ...
    def sensitivity_filter(self):
        ref = self._ref_permission
        return ref.sensitivity_filter if ref is not None else None
    ...
    def taxa(self):
        ref = self._ref_permission
        return ref.taxons_filter if ref is not None else []
